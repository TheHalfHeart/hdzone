CKEDITOR.dialog.add( 'syntaxhighlightDialog', function( editor ) {
	...
	var getOptionsForString=function(textareaElement) {
		var options=getDefaultOptions();
		var optionsString=textareaElement.getAttribute('class');
		if(optionsString) {
			...
			var codeTitle=textareaElement.getAttribute('title');
			if (codeTitle) {
	...
	return {
		...
		onShow : function() {
			...
			var textareaElement=element&&element.getAscendant('textarea',true);
			var text='';
			var optionsObj=null;
			if(textareaElement) {
				code=textareaElement.getHtml().replace(/<br>/g,"\n").replace(/&nbsp;/g,' ').replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&quot;/g,'"').replace(/&amp;/g,'&');
				optionsObj=getOptionsForString(textareaElement);
		...
		onOk : function() {
			...
			if(textareaElement) {
				textareaElement.setAttribute('class', optionsString);
				(ti!=false) ? textareaElement.setAttribute('title', ti) : textareaElement.removeAttribute('title');
				textareaElement.setText(data.code);
			} else {
				var newElement=new CKEDITOR.dom.element('textarea');
