...
<div class="box">
    <div class="heading">
        <h1><img alt="" src="public/admin/image/home.png">Danh Sách Sản Phẩm (<?php echo $this->pagination->total_record; ?>)</h1>
        <div class="buttons">
            <a class="button" onclick="return jsAdmin.redirect('product/post_add');">Thêm Mới</a>
            ...
            <table class="list">
                ...
                <tbody>
                    <tr class="filter">
                        ...
                        <td class="center">
                            <select id="post_status" style="width: 100%;">
                                <option value=""></option>
                                <option value="0" <?php _selected($filter, 'post_status', '0'); ?>>No</option>
                                <option value="1" <?php _selected($filter, 'post_status', '1'); ?>>Yes</option>
                            </select>
                        </td>
                         <td class="left">
                        ...
                        <td class="left" style="display: none ">
                             <select id="post_ref_brand_id" style="display: none ">
                                 
                             </select>
                        </td>
                        <td class="left" style="display: none ">
                             <select id="post_ref_manu_id" style="display: none ">
                        ...
                        <td align="right">
                            <a class="button" onclick=" jsAdmin.redirect('product/post_lists'); return false;">Reset</a>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="left">
                            <select id="cate_parent_id_wrapper_<?php echo $item['post_id']; ?>" data-id="<?php echo $item['post_id']; ?>" data-field="post_ref_cate_id" class="quick-edit">
                            </select>
                        </td>
                        <td class="left" style="display: none ">
                            <select id="brand_parent_id_wrapper_<?php echo $item['post_id']; ?>" data-id="<?php echo $item['post_id']; ?>" data-field="post_ref_brand_id" class="quick-edit">
                            </select>
                        </td>
                        <td class="left" style="display: none ">
                            <select id="manu_parent_id_wrapper_<?php echo $item['post_id']; ?>" data-id="<?php echo $item['post_id']; ?>" data-field="post_ref_manu_id" class="quick-edit">
                            </select>
                        </td>
...
<script language="javascript">
    $(document).ready(function()
    {
        ...
        showParentCate(cate, 0, 0, <?php echo (int)$item['post_ref_cate_id']; ?>);
        $('#cate_parent_id_wrapper_'+<?php echo (int)$item['post_id']; ?>).html(str);
        <?php } ?>
            
        // Manu
        str = '<option value="">TẤT CẢ</option>';
        showParentManu(manu, 0, 0, <?php echo (int)$filter['post_ref_manu_id']; ?>);
        $('#post_ref_manu_id').html(str);
        
        <?php foreach ($data as $item){ ?>
        str = '<option value="">----</option>';
        showParentManu(manu, 0, 0, <?php echo (int)$item['post_ref_manu_id']; ?>);
        $('#manu_parent_id_wrapper_'+<?php echo (int)$item['post_id']; ?>).html(str);
        <?php } ?>
        
        // Brand
        str = '<option value="">TẤT CẢ</option>';
        showParentBrand(brand, 0, 0, <?php echo (int)$filter['post_ref_brand_id']; ?>);
        $('#post_ref_brand_id').html(str);
        
        <?php foreach ($data as $item){ ?>
        str = '<option value="">----</option>';
        showParentBrand(brand, 0, 0, <?php echo (int)$item['post_ref_brand_id']; ?>);
        $('#brand_parent_id_wrapper_'+<?php echo (int)$item['post_id']; ?>).html(str);
        <?php } ?>
        
        jsAdmin.changeTitle('Danh Sách Sản Phẩm');
            ...
            if (list_id)
            {
                jConfirm('Bạn có chắc muốn xóa những trang này?', 'Xác nhận xóa', function (r){
                    if (r)
                    {
                        ...
                        jsAdmin.sendAjax('post', 'text', data, 'product/post_delete', function (result)
                        {
                            result = trim(result);
                            if (result == '100'){
        ...
        $('.quick-edit').change(function()
        {
            ...
            if (!inArray(data.field, new Array('post_ref_cate_id', 'post_ref_brand_id', 'post_ref_manu_id'))){
                return false;
            }
            
            jsAdmin.sendAjax('post', 'text', data, 'product/post_quick_edit', function (result)
            {
                result = trim(result);
        ...
        $('.edit-click').click(function(){
            jsAdmin.redirect('product/post_edit?post_id='+$(this).attr('idval'));
            return false;
        });
        ...
        function filter()
        {
                ...
                jsAdmin.event.filter(data, 'product/post_lists');
        }
    });
</script>