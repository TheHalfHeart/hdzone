...
<div class="box">
                ...
                <table class="form">
                    <tbody>
                        ...
                        <tr>
                         ...
                         <tr>
                            <td>
                                <strong>Giá <span class="required">(*)</span></strong>
                                <span class="help">Giá sản phẩm</span>
                            </td>
                            <td>
                                <input type="text" onkeypress="return onlyNumbers(event);" id="post_price" value="<?php _val($filter, 'post_price'); ?>" size="20"/>
                                <?php showErrorLang($errors, 'post_price'); ?>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <strong>Số Sao <span class="required">(*)</span></strong>
                                <span class="help">Đánh giá số sao của sản phẩm, tối đa 5 sao</span>
                            </td>
                            <td>
                                <input type="text" onkeypress="return onlyNumbers(event);" id="post_star" value="<?php _val($filter, 'post_star'); ?>" size="20"/>
                                <?php showErrorLang($errors, 'post_star'); ?>
                            </td>
                        </tr>
                        <tr>
                            <td>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="multilang" type="ckeditor" toolbar="full" ck-id="<?php echo $filter['post_id']; ?>" ck-start-path="<?php echo date('Y/m/d/',$filter['post_add_date_int']).$filter['post_id']; ?>" ck-module="product" id="post_content" name="post_content" value="<?php _lang_val($filter, 'post_content'); ?>"></div>
                            </td>
                        </tr>
                    </tbody>
                </table>
                    ...
                    <tbody>
                        ...
                        <tr style="display: none ">
                            <td>
                                ...
                                <span class="help">Nhà sản xuất của tin</span>
                            </td>
                            <td>
                                <select id="post_ref_manu_id"></select>
                            </td>
                        </tr>
                        <tr style="display: none ">
                            <td>
                                <strong>Thương Hiệu</strong>
                                <span class="help">Thương hiệu của tin</span>
                            </td>
                            <td>
                                <select id="post_ref_brand_id"></select>
                            </td>
                        </tr>
                        <tr>
                            <td>
            ...
            <div id="tab-image">
                <table class="form">
                    <tbody>
                        <tr>
                            <td>
                                <strong>Hình Đại Diện Nhỏ</strong>
                                ...
                                <span class="help">- Hãy resize hình ảnh về mức 400x300 (dài x rộng) để website chạy nhanh hơn</span>
                            </td>
                            <td>
                    ...
                    <table class="form">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php echo $this->config->base_url($filter['post_slug_vi'].'-'.$filter['post_id'].'.html'); ?>
                                </td>
                            </tr>
                        </tbody>
                    </table>
    ...
    <input type="hidden" id="interval-product-post-editting" value="product-cate-editting"/>
</div>
<?php $this->product_cate_model->to_js_list($cate); ?>
<?php $this->product_brand_model->to_js_list($brand); ?>
<?php $this->product_manu_model->to_js_list($manu); ?>
<script language="javscript" type="text/javascript">
    $(document).ready(function()
    {
        // INIT MENU
        var str = '';
        function showParentCate(cateList, parent_id, dem, current_id)
        {
            ...
            if (arrayForeach.length > 0)
            {
                for (i = 0; i < arrayForeach.length; i++)
                {
                    ...
                    str += '</option>';
                    showParentCate(cateContinue, arrayForeach[i].cate_id, dem+1, current_id);
                }
            } 
        }
        
        function showParentBrand(brandList, parent_id, dem, current_id)
        {
            var arrayForeach = new Array();
            var brandContinue = new Array();

            for(var i = 0; i < brandList.length; i++){
                if (brandList[i].brand_ref_parent_id == parent_id){
                    arrayForeach.push(brandList[i]);
                }
                else{
                    brandContinue.push(brandList[i]);
                }
            }
            ...
            if (arrayForeach.length > 0)
            {
                for (i = 0; i < arrayForeach.length; i++)
                {
                    str += '<option value="'+arrayForeach[i].brand_id +'"';
                    
                    if (arrayForeach[i].brand_id == current_id){
                        str += ' selected style="background:gray" ';
                    }
                    ...
                    str += arrayForeach[i].<?php echo lang_field('brand_title_short'); ?>;
                    str += '</option>';
                    showParentBrand(brandContinue, arrayForeach[i].brand_id, dem+1, current_id);
                }
            } 
        }
        
        function showParentManu(manuList, parent_id, dem, current_id)
        {
            var arrayForeach = new Array();
            var manuContinue = new Array();

            for(var i = 0; i < manuList.length; i++){
                if (manuList[i].manu_ref_parent_id == parent_id){
                    arrayForeach.push(manuList[i]);
                }
                else{
                    manuContinue.push(manuList[i]);
                }
            }
            ...
            if (arrayForeach.length > 0)
            {
                for (i = 0; i < arrayForeach.length; i++)
                {
                    str += '<option value="'+arrayForeach[i].manu_id +'"';
                    
                    if (arrayForeach[i].manu_id == current_id){
                        str += ' selected style="background:gray" ';
                    }
                    ...
                    str += arrayForeach[i].<?php echo lang_field('manu_title_short'); ?>;
                    str += '</option>';
                    showParentManu(manuContinue, arrayForeach[i].manu_id, dem+1, current_id);
                }
            } 
        }
        
        var str = '<option value="0">Kho Lưu Trữ</option>';
        showParentCate(cate, 0, 0, <?php echo (int)$filter['post_ref_cate_id']; ?>);
        $('#post_ref_cate_id').append(str);
        
        var str = '<option value="0">Kho Lưu Trữ</option>';
        showParentBrand(brand, 0, 0, <?php echo (int)$filter['post_ref_brand_id']; ?>);
        $('#post_ref_brand_id').append(str);
        
        var str = '<option value="0">Kho Lưu Trữ</option>';
        showParentManu(manu, 0, 0, <?php echo (int)$filter['post_ref_manu_id']; ?>);
        $('#post_ref_manu_id').append(str);
        
        // Lang
        jsAdmin.loadLang();
        
        // Title Page
        jsAdmin.changeTitle('Sửa Sản Phẩm');
        
        // Menu
        jsAdmin.changeMenu('product/post_lists');
        
        // Ckfinder
        jsAdmin.ckfinder.connectorPath = jsAdmin.linkApi + '/ckfinder/product/connector/<?php echo $filter['post_id']; ?>/Images';
        ...
        $('#click-save').click(function()
        {
            var data = 
            {
                ...
                post_ref_tags_id    : jsProduct.getListTags().join(' '),
                post_ref_related    : jsProduct.getListRelated().join(' '),
                post_ref_cate_id    : trim($('#post_ref_cate_id').val()),
                post_ref_brand_id    : trim($('#post_ref_brand_id').val()),
                post_ref_manu_id    : trim($('#post_ref_manu_id').val()),
                post_image          : trim($('#image_server').val()),
                post_image_large    : trim($('#image_server_large').val()),
                post_price          : trim($('#post_price').val()),
                post_star          : trim($('#post_star').val()),
            ...
            jsAdmin.sendAjax('post', 'text', data, 'product/post_edit?post_id=<?php echo $filter['post_id']; ?>', function (result)
            {
                result = trim(result);
                if (result == '100'){
                    jAlert('Sửa thành công', 'Thành công', function (){
                        jsAdmin.redirect('product/post_edit?post_id=<?php echo $filter['post_id']; ?>');
                    });
                }
                else if (result == '102'){
        ...
        {
            if (!isEmpty($('#interval-product-post-editting').val())){
                jsAdmin.sendHideAjax('get', 'text', {post_id:<?php echo $filter['post_id']; ?>}, 'product/post_editting');
            }
            else{
                clearInterval(jsAdmin.editting);
            }
