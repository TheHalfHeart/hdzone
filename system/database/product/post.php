...
class Product_post_table extends MY_Table 
{
    //-----------------------------
    // Thông Tin Table
    protected $_db_conf     = 'default';
    protected $_tb_name     = 'product_post';
    ...
    {
        $str = '';
        new_table('product', 'cate')->getListCateIdById($id, $str);
    ...
    function where_brand_id($id)
    {
        $str = '';
        new_table('product', 'brand')->getListCateIdById($id, $str);
        $str = trim($str);
        $arr = preg_split('/\s+/', trim($str));
        if ($str && $arr) {
            $this->begin();
            foreach ($arr as $key => $val) {
                if ($key > 0) {
                    $this->where_or('post_ref_brand_id', (int) $val);
                }
                $this->where('post_ref_brand_id', (int) $val);
            }
            $this->end();
        }
    }
    
    function where_manu_id($id)
    {
        $str = '';
        new_table('product', 'manu')->getListCateIdById($id, $str);
        $str = trim($str);
        $arr = preg_split('/\s+/', trim($str));
        if ($str && $arr) {
            $this->begin();
            foreach ($arr as $key => $val) {
                if ($key > 0) {
                    $this->where_or('post_ref_manu_id', (int) $val);
                }
                $this->where('post_ref_manu_id', (int) $val);
            }
            $this->end();
        }
    }
