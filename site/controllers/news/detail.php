...
class Detail extends CI_Controller {
    ...
    { 
        ...
        $current_cate = $this->news_cate_model->getDetail(array(
        ...
        if (isset($current_cate['cate_ref_parent_id'])){
            $cate_parent = $this->news_cate_model->getDetail(array(
                'cate_id' => $current_cate['cate_ref_parent_id']
            ));
        }
        
        $this->news_post_model->setWhere(array(
            'id_related' => $post['post_id'],
            'post_ref_cate_id' => $post['post_ref_cate_id']
        ));
        
        $related = $this->news_post_model->getList(array(
            'limit' => '20',
            'start' => '0'
        ));
        ...
        if ($post['post_ref_tags_id']){
            foreach (explode(' ', trim($post['post_ref_tags_id'])) as $id){
                $tmp = $this->news_tags_model->getDetail(array(
                    'tags_id' => $id
                ));
                if ($tmp){
                    $tags[] = $tmp;
        ...
        $metas = array(
            ...
            'page_url'     => $this->config->base_url('san-pham/'.$post['post_slug'._LANG].'-'.$post['post_id'].'.html'),
            'page_type' => 'article',
        ...
        $this->load->view('news/detail', array(
            'metas' => $metas,
            'tags' => $tags,
            'related' => $related,
            'cate' => $current_cate,
