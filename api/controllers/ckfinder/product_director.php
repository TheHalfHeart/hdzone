...
class Product_director extends CI_Controller 
{
    function connector($id = 0, $type = 'Images') 
    {
        define('ROOT_CKFINDER', './public/ckfinder/');
        define('FOLDER', 'product_director');
        define('BASE_URL_FULL', $this->config->base_url('upload/product_director'));
        ...
        {
            $director = new_table('product', 'director')->select('director_add_date_int')->where('director_id', (int)$id)->execute()->get_result(0);
            if (!$director){
                define('CAN_USE_CKFINDER', false);
            }
            else
    ...
    function html($id = false) {
        $this->load->view('ckfinder/product_director_view', array(
            'id' => $id
        ));
    }
}
