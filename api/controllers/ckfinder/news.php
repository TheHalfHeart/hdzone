...
class News extends CI_Controller 
{
    function connector($id = 0, $type = 'Images') 
    {
        ...
        define('FOLDER', 'news');
        define('BASE_URL_FULL', $this->config->base_url('upload/news'));
        ...
        {
            $post = new_table('news', 'post')->select('post_add_date_int,post_add_user_username,post_timer_date_time_int')->where('post_id', (int)$id)->execute()->get_result(0);
    ...
    function html($id = false) {
        $this->load->view('ckfinder/news_view', array(
            'id' => $id
        ));
    }
}
