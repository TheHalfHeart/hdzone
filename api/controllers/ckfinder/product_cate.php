...
class Product_cate extends CI_Controller 
{
    function connector($id = 0, $type = 'Images') 
    {
        ...
        define('FOLDER', 'product_cate');
        define('BASE_URL_FULL', $this->config->base_url('upload/product_cate'));
        ...
        {
            $cate = new_table('product', 'cate')->select('cate_add_date_int')->where('cate_id', (int)$id)->execute()->get_result(0);
            if (!$cate){
                define('CAN_USE_CKFINDER', false);
            }
            else
    ...
    function html($id = false) {
        $this->load->view('ckfinder/product_cate_view', array(
            'id' => $id
        ));
    }
}
