...
class Product_type extends CI_Controller 
{
    function connector($id = 0, $type = 'Images') 
    {
        ...
        define('FOLDER', 'product_type');
        define('BASE_URL_FULL', $this->config->base_url('upload/product_type'));
        ...
        {
            $type = new_table('product', 'type')->select('type_add_date_int')->where('type_id', (int)$id)->execute()->get_result(0);
            if (!$type){
                define('CAN_USE_CKFINDER', false);
            }
            else
    ...
    function html($id = false) {
        $this->load->view('ckfinder/product_type_view', array(
            'id' => $id
        ));
    }
}
